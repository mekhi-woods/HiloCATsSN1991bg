...
def dataset_process(data_set, processes = ['mass'], mag_unc_max=0, flux_unc_max=0,
                    use_saved=True, show_plots=True, save=False, quiet=False):
    save_loc = CONSTANTS['salt_'+data_set.lower()+'_loc']
    save_txt = save_loc + data_set.lower() + '_salt_saved.txt'
    processingArgs = {'data_set': data_set, 'mag_unc_max': mag_unc_max, 'flux_unc_max': flux_unc_max, 'quiet': False}
    ...
    for obj in objs:
        ...
        if data_set == 'CSP':
            ...
            objs[obj]['filters'][np.where(objs[obj]['filters'] == 'Jrc2')[0]] = 'cspjd'
    ...
    return
def indivisual_process(SN, data_set, processes = ['mass'], mag_unc_max=0, flux_unc_max=0,
                       use_saved=True, show_plots=True, save=False, quiet=False):
    save_loc = CONSTANTS['salt_'+data_set.lower()+'_loc']
...
def salt3_fit(objs, plot_save_loc='../default/', plot_data=True, save_plot=True):
    ...
    for obj in objs:
        ...
        try:
            ...
            # Fit data to model
            model.set(z=objs[obj]['z'], t0=objs[obj]['time'][np.where(objs[obj]['flux'] == np.max(objs[obj]['flux']))[0][0]])  # set the model's redshift.
