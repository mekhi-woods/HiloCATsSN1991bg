...
def snpy_combined_process():
    ...
    for path in glob.glob(CONSTANTS['csp_data_loc']+'*.txt'):
        if path.split('/')[-1][2:-9] in np.genfromtxt(CONSTANTS['csp_91bg_txt'], dtype=str, delimiter=', '):
            paths.append(path)
    fit_args = {'save_loc': CONSTANTS['csp_saved_loc'], 'use_saved': True, 'snpy_plots': True, 'save_plots': True, 'quiet': False}
    cspParams = gen.snpy_fit(paths=paths, **fit_args)
    ...
    fit_args = {'save_loc': CONSTANTS['combined_saved_loc'], 'use_saved': True, 'snpy_plots': True, 'save_plots': True, 'quiet': False}
...
def salt3_atlas_process():
    ...
    objs = alt_data_proccesser(data_set='ATLAS', quiet=True)
    for obj in objs:
...
def salt3_ztf_process():
    objs = gen.alt_data_proccesser(data_set='ZTF', quiet=False)
    ...
    fit_args = {'plot_save_loc': CONSTANTS['salt_ztf_loc']+'plots/', 'plot_data': True, 'save_plot': False}
...
def plotter(choices):
    for data_set in choices:
        ...
        res_plot_args = {'save_loc': CONSTANTS[data_set+'_saved_loc'], 'sigma': [1, 1],
                         'labels': True, 'raw': False, 'extra_info': True, 'ignore_type': [], 'save_plot': True}
        gen.residual_plotter(path=CONSTANTS[data_set+'_saved_loc']+data_set+'_saved_cut.txt', x_params=['z', 'Redshift'], **res_plot_args)
        gen.residual_plotter(path=CONSTANTS[data_set+'_saved_loc']+data_set+'_saved_cut.txt', x_params=['host_mass', 'Host Mass'], **res_plot_args)
...
# -------------------------------------------------------------------------------------------------------------------- #
    ...
    # fit_reviewer()
    # gen.sample_cutter('combined_saved_loc')
    # plotter(['combined'])
